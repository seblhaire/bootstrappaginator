...
class BootstrapPaginatorProvider {
        ...
        if ($this->options['nbpages'] > 1) {
            ...
            if ($this->options['nbpages'] <= $this->options['max_pages_without_dots'] || $this->options['withoutdots']) {
                ...
                if ($this->currentitem <= (1 + (2 * $this->options['items_before_after_current']))) {
                    $str .= $this->boucle(2, (3 + (2 * $this->options['items_before_after_current']))) . $this->dots();
                } else if ($this->currentitem >= ($this->options['nbpages'] - (2 * $this->options['items_before_after_current']))) {
                    //pagination: l<=9; p<=5; p>= l-4; default...
                    $str .= $this->dots() . $this->boucle(($this->options['nbpages'] - (2 + (2 * $this->options['items_before_after_current']))), ($this->options['nbpages'] - 1));
                } else {
    ...
    private function alpha() {
        ...
        for ($i = 65; $i < 91; $i++) {
            ...
            $str .= "><a class=\"" . $this->options['linkclass'] . "\" href=\"" .
                    route($this->currentroute, array_merge($this->options['params'], [
                        $this->options['initialparam'] => chr($i)])) .
                    "\" >" . chr($i) . "</a></li>";
        }
        $str .= "</ul></nav>";
        return $str;
